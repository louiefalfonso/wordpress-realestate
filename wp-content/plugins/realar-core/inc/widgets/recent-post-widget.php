...
class realar_recent_posts_widget extends WP_Widget {
        ...
        function __construct() {
            ...
            parent::__construct(
                ...
                'realar_recent_posts_widget',

                // Widget name will appear in UI
                esc_html__( 'Realar :: Recent Posts', 'realar' ),

                // Widget description
                array(
                    'classname'                     => '',
                    'customize_selective_refresh'   => true,
                    'description'                   => esc_html__( 'Add Recent Posts Widget', 'realar' ),
                )
            );
        }
                            ...
                            echo '<div class="recent-post">';
                                    ...
                                    echo '<div class="media-img">';
                                        echo '<a href="'.get_the_permalink().'">';
                                            echo realar_img_tag( array(
                                                "url"   => esc_url( get_the_post_thumbnail_url( null, 'realar_85X85' ) ),
                                            ) );
                                        echo '</a>';
                                    echo '</div>';
                                }
                                echo '<div class="media-body">';
                                    ...
                                    echo '<div class="recent-post-meta">'; 
                                        if($show_date){
                                            echo '<a href="'.esc_url( get_the_permalink() ).'"><i class="fa-solid fa-calendar-days"></i>'.esc_html( get_the_date() ).'</a>';
                                        }
                                    echo '</div>';
        ...
        public function form( $instance ) {
           ...
           //Post title Count
            ...
            <p>
                <label for="<?php echo $this->get_field_id( 'post_count' ); ?>"><?php _e( 'Number of Posts to show:' ,'realar'); ?></label>
            ...
            <p>
                <label for="<?php echo $this->get_field_id( 'post_title_count' ); ?>"><?php _e( 'Number of title word to show:' ,'realar'); ?></label>
    ...
    } // Class realar_recent_posts_widget ends here


    // Register and load the widget
    function realar_recent_posts_load_widget() {
        register_widget( 'realar_recent_posts_widget' );
    }
    add_action( 'widgets_init', 'realar_recent_posts_load_widget' );