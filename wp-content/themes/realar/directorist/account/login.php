...
?>
<div class="directorist-login-wrapper directorist-w-100 space-top space-bottom">
            ...
            <div class="directorist-col-md-6 directorist-offset-md-3">
                <div class="atbdp_login_form_shortcode">
					...
					<?php if ( directorist_is_email_verification_enabled() && ! empty( $_GET['send_verification_email'] ) ) : ?>
						<p class="directorist-alert directorist-alert-success"><span>
							<?php echo wp_kses( __( "Thank you for requesting a new verification email. Please check your inbox and verify to complete the registration.<br><br>If you still can't find it, please check your spam folder. And please contact if you are still having trouble.", 'realar' ), array( 'br' => array() ) ); ?>
						</span></p>
					<?php endif; ?>
					...
					if ( is_email( $user_email ) && ! empty( $key ) ) {
						...
						if ( ! $user ) { ?>
							<p class="directorist-alert directorist-alert-danger">
								<?php esc_html_e( 'Sorry! user not found', 'realar' ); ?>
							</p>
						<?php } else {
							...
							if ( is_wp_error( $is_valid_password_reset_key ) ) {
								?><p class="directorist-alert directorist-alert-danger">
									<?php echo esc_html($is_valid_password_reset_key->get_error_message()); ?>
								...
								if ( ! empty( $_POST['directorist_reset_password'] ) && directorist_verify_nonce( 'directorist-reset-password-nonce', 'reset_password' ) ) :
									...
									if ( empty( $password_1 ) || empty( $password_2 ) ) : ?>
										<p class="atbd_reset_warning directorist-alert directorist-alert-danger"><?php echo esc_html__( 'Passwords cannot be empty.', 'realar' ); ?></p>
									<?php elseif ( $password_1 !== $password_2 ) : ?>
										<p class="atbd_reset_error directorist-alert directorist-alert-danger"><?php echo esc_html__( 'Passwords do not match!', 'realar' ); ?></p>
									<?php else :
										...
										<p class="atbd_reset_success directorist-alert directorist-alert-success"><?php echo wp_kses( sprintf(
											__( 'Password changed successfully. Please <a href="%s">click here to login</a>.', 'realar' ),
								...
								if ( ! empty( $_GET['confirm_mail'] ) ) {
									...
									<div class="directorist-alert directorist-alert-success">
										<?php echo wp_kses( sprintf(
											__( 'Email verification successful. Please <a href="%s">click here to login</a>.', 'realar' ),
											esc_url( ATBDP_Permalink::get_login_page_url() )
						...
						$log_username        = get_directorist_option( 'log_username', __( 'Username or Email Address', 'realar' ) );
						$log_password        = get_directorist_option( 'log_password', __( 'Password', 'realar' ) );
						$display_rememberMe  = get_directorist_option( 'display_rememberme', 1 );
						$log_rememberMe      = get_directorist_option( 'log_rememberme', __( 'Remember Me', 'realar' ) );
						$log_button          = get_directorist_option( 'log_button', __( 'Log In', 'realar' ) );
						$display_recpass     = get_directorist_option( 'display_recpass', 1 );
						$recpass_text        = get_directorist_option( 'recpass_text', __( 'Recover Password', 'realar' ) );
						$recpass_desc        = get_directorist_option( 'recpass_desc', __( 'Lost your password? Please enter your email address. You will receive a link to create a new password via email.', 'realar' ) );
						$recpass_username    = get_directorist_option( 'recpass_username', __( 'E-mail:', 'realar' ) );
						$recpass_placeholder = get_directorist_option( 'recpass_placeholder', __( 'eg. mail@example.com', 'realar' ) );
						$recpass_button      = get_directorist_option( 'recpass_button', __( 'Get New Password', 'realar' ) );
						$reg_text            = get_directorist_option( 'reg_text', __( "Don't have an account?", 'realar' ) );
						$reg_url             = ATBDP_Permalink::get_registration_page_link();
						$reg_linktxt         = get_directorist_option( 'reg_linktxt', __( 'Sign Up', 'realar' ) );
						...
						if ( isset( $_POST['action'] ) && 'reset' === $_POST['action'] && directorist_verify_nonce() ) :
							...
							if ( empty( $email ) ) {
								$error = __( 'Email address cannot be empty.', 'realar' );
							} else if ( ! is_email( $email ) ) {
								$error = __( 'Invalid e-mail address.', 'realar' );
							} else if ( ! email_exists( $email ) ) {
								$error = __( 'There is no user registered with that email address.', 'realar' );
							} else {
								$user      = get_user_by( 'email', $email );
								/* translators: %s: site name */
								$subject   = esc_html( sprintf( __( '[%s] Reset Your Password', 'realar' ), get_option( 'blogname', 'display' )  ));
								$title     = esc_html__( 'Password Reset Request', 'realar' );
								...
								$message = sprintf(
									    __( 'Someone has requested a password reset for the following account:
									          ...
									          If this was a mistake, just ignore this email and nothing will happen.', 'realar' ),
									    $site_name,
									    $user->user_login,
								...
								if ( $error ) {
									$success = __( 'A password reset email has been sent to the email address on file for your account, but may take several minutes to show up in your inbox.', 'realar' );
								} else {
									$error = __( 'Something went wrong, unable to send the password reset email. If the issue persists please contact with the site administrator.', 'realar' );
								}

							}

							if ( ! empty( $error ) ) {
								echo '<div class="message"><p class="error directorist-alert directorist-alert-danger">' . wp_kses( sprintf( __( '<strong>ERROR: </strong> %s', 'realar' ), esc_html( $error ) ), array( 'strong' => array() ) ) . '</p></div>';
							}
