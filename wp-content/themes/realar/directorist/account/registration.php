...
?>
<div class="directorist-registration-wrapper directorist-w-100 space-top space-bottom">
				...
				<div class="add_listing_title atbd_success_mesage">
					...
					if ( ! empty( $_GET['registration_status'] ) && true == $_GET['registration_status'] ) {
						if ( empty( $display_password_reg ) ) {
							?>
							<p style="padding: 20px" class="alert-success directorist-alert directorist-alert-success"><span><?php directorist_icon( 'las la-check' ); ?> <?php esc_html_e('Go to your inbox or spam/junk and get your password.', 'realar'); ?>
								<?php
								$output = sprintf( __( 'Click %s to login.', 'realar' ), '<a href="' . ATBDP_Permalink::get_login_page_link() . '"><i style="color: red">' . __( 'Here', 'realar' ) . '</i></a>' );
								echo wp_kses_post( $output );
								?>
							</span></p>
						<?php } else { ?>
							<!--registration succeeded, so show notification -->
							<p style="padding: 20px" class="alert-success directorist-alert directorist-alert-success"><span><?php directorist_icon( 'las la-check' ); ?> <?php esc_html_e('Registration completed. Please check your email for confirmation.', 'realar'); ?>
								<?php
								$output = sprintf( __('Or click %s to login.', 'realar' ), '<a href="' . ATBDP_Permalink::get_login_page_link() . '"><span style="color: red">' . __( 'Here', 'realar' ) . '</span></a>' );
								echo wp_kses_post( $output );
								?>
							</span></p>
						<?php
					...
					<form action="<?php the_permalink(); ?>" method="post">
						...
						<?php if ( ! empty( get_directorist_option( 'display_user_type' ) ) ) {
							...
							<div class="atbd_user_type_area directory_regi_btn directorist-radio directorist-radio-circle directorist-mb-15">
								<input id="author_type" type="radio" name="user_type" value="author" <?php echo esc_attr( $author_checked ); ?>><label for="author_type" class="directorist-radio__label"><?php esc_html_e( 'I am an author', 'realar' ); ?>
							</div>
							<?php } ?>
							<?php if ( empty( $user_type ) || 'general' == $user_type ) { ?>
							<div class="atbd_user_type_area directory_regi_btn directorist-radio directorist-radio-circle directorist-mb-15">
								<input id="general_type" type="radio" name="user_type" value="general" <?php echo esc_attr( $general_checked ); ?>><label for="general_type" class="directorist-radio__label"><?php esc_html_e( 'I am a user', 'realar' ); ?>
							</div>
							<?php } ?>
						<?php } ?>
